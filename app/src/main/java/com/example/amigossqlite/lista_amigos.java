...
public class lista_amigos extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.fabAgregarAmigo).setOnClickListener(v -> {
        ...
        ltsAmigos.setOnItemClickListener((parent, view, position, id) -> {
            amigos amigo = alAmigos.get(position);
            Intent intent = new Intent(this, chats.class);
            intent.putExtra("idAmigo", amigo.getIdAmigo());
            intent.putExtra("nombre", amigo.getNombre());
            intent.putExtra("to", amigo.getToken());
            intent.putExtra("urlFoto", amigo.getFoto());
            startActivity(intent);
        });
    ...
    public void onCreateContextMenu(ContextMenu menu, View v, ContextMenu.ContextMenuInfo menuInfo) {
        ...
        menu.setHeaderTitle(alAmigos.get(posicion).getNombre());
    }

    @Override
    public boolean onContextItemSelected(@NonNull MenuItem item) {
        amigos amigo = alAmigos.get(posicion);
        if (item.getItemId() == R.id.mnxModificar) {
            Intent intent = new Intent(this, MainActivity.class);
            intent.putExtra("accion", "modificar");
            intent.putExtra("idAmigo", amigo.getIdAmigo());
            intent.putExtra("nombre", amigo.getNombre());
            intent.putExtra("direccion", amigo.getDireccion());
            intent.putExtra("telefono", amigo.getTelefono());
            intent.putExtra("email", amigo.getEmail());
            intent.putExtra("dui", amigo.getDui());
            intent.putExtra("urlFoto", amigo.getFoto());
            startActivity(intent);
            return true;
        } else if (item.getItemId() == R.id.mnxEliminar) {
            FirebaseDatabase.getInstance().getReference("amigos")
                    .child(amigo.getIdAmigo()).removeValue()
                    .addOnSuccessListener(unused -> mostrarMsg("Amigo eliminado"))
                    .addOnFailureListener(e -> mostrarMsg("Error al eliminar: " + e.getMessage()));
            return true;
        }
